...
public class Keyboard {
    ...
    public Keyboard(Context context, int defaultHeight, int xmlLayoutResId, int modeId, float kbHeightPercent) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R && context instanceof InputMethodService) {
            InputMethodService ims = (InputMethodService) context;
            if (ims.getWindow() != null && ims.getWindow().getContext() != null) {
                WindowMetrics wm = ims.getWindow().getContext()
                        .getSystemService(WindowManager.class)
                        .getCurrentWindowMetrics();
                Insets insets = wm.getWindowInsets()
                        .getInsetsIgnoringVisibility(WindowInsets.Type.systemBars() | WindowInsets.Type.displayCutout());
                mDisplayWidth = wm.getBounds().width() - insets.left - insets.right;
            }
        }
